...
function isValidAtomicRule(rule) {
  ...
  for (let i = 1; i < decls.length; i++) {
    const current = groupedOrder.indexOf(decls[i].prop);
    const prev = groupedOrder.indexOf(decls[i - 1].prop);
    if (current < prev) {
      const loc = {
        start: {
        ...
        try {
          ...
          if (!isValid) {
            return context.report({
              node,
              messageId: "sort-declarations-grouped",
              loc,
              fix: fixer =>
                fix({
                  rule: root,
...
function sortDeclarations(declarations) {
  return declarations
    .slice()
    .sort((declA, declB) => (groupedOrder.indexOf(declA.prop) > groupedOrder.indexOf(declB.prop) ? 1 : -1));
}

module.exports = {
  meta: {
    docs: {
      description: "Styles are sorted grouped.",
      category: "Fill me in",
      recommended: false
    },
    messages: {
      "sort-declarations-grouped":
        "Declarations should be sorted grouped."
    },
    fixable: "code"
  },
  create
