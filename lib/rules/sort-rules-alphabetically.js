 ...
 */
function isValidAtomicRule({ rule, rootLoc }) {
...
function isValidRule({ rule, rootLoc }) {
  ...
  const { isValid, loc } = rule.nodes.reduce(
    (map, node) => {
      return node.type === "rule" ? isValidRule({ rule: node, rootLoc }) : map;
    },
    { isValid: true }
  );
...
module.exports = {
  meta: {
    ...
    messages: {
      "sort-rules-alphabetically": "Rules should be sorted alphabetically."
    },
    fixable: "code"
  },
  create
